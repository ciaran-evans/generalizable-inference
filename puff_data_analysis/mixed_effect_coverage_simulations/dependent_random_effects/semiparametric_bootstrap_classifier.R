...
simulate_test_data <- function(original_test_data){
  ...
  for(k in 1:7){
    ...
    re_puff <- rnorm(1, 0, 0.1)
    re_nonpuff <- rnorm(1, 0, 0.1)
    ...
    test_data <- test_data %>%
      rbind(((original_test_data %>%
                ...
                mutate(smoothness = smoothness + re_puff))*matrix(rep(obs_y, 6), ncol=6) +
               (original_test_data %>%
                  filter(puff == 0) %>%
                  select(conv_area_scaled, conv_perim_scaled,
                         noise_scaled, snr_scaled,
                         intens_ratio_scaled, smoothness) %>%
                  sample_n(7500, replace=T) %>%
                  mutate(smoothness = smoothness + re_nonpuff))*matrix(rep(1-obs_y, 6), ncol=6)) %>%
              as.data.frame() %>%
              mutate(puff = obs_y,
                     cell = k))
  }
...
for(samp in 1:num_boot){
  ...
  if(!is.na(bootstrap_puff_est) & bootstrap_puff_est > 0 & bootstrap_puff_est < 1){
    ...
    bootstrap_puff_sds[samp] <- suppressMessages(bootstrap_test_data %>%
                                                   group_by(cell) %>%
                                                   summarize(m = mean(smoothness[puff == 1])) %>%
                                                   pull(m) %>%
                                                   sd())
    bootstrap_nonpuff_sds[samp] <- suppressMessages(bootstrap_test_data %>%
                                                      group_by(cell) %>%
                                                      summarize(m = mean(smoothness[puff == 0])) %>%
                                                      pull(m) %>%
                                                      sd())
...
write(output, 
            file=paste("semiparametric_bootstrap_",
                       ci_rep, ".txt", sep = ""))
