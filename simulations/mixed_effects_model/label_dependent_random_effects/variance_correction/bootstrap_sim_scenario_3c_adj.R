...
simulate_training_data <- function(train_pi, n_obs){
  ...
  train_data <- rsn(n_obs, 0, 2, 3)*(1-obs_y) + 
...
for(ci_rep in 1:num_ci){
  tryCatch({
    ...
    # variance calibration step
    bootstrap_puff_sds <- c()
    bootstrap_puff_sds_est <- c()
    bootstrap_nonpuff_sds <- c()
    bootstrap_nonpuff_sds_est <- c()
    for(samp in 1:num_boot){
      ...
      if(!is.na(bootstrap_puff_est) & bootstrap_puff_est > 0 & bootstrap_puff_est < 1){
        ...
        bootstrap_me_mod_puff <- bootstrap_test_data %>%
        ...
        bootstrap_me_mod_nonpuff <- bootstrap_test_data %>%
          mutate(pred = bootstrap_preds, 
                 pred = pred*bootstrap_w1/(pred*bootstrap_w1 + 
                                             (1 - pred)*bootstrap_w2)) %>%
          lmer(resid_dist ~ (1|cell), weights = 1-pred,
               data=.)
        
        bootstrap_puff_sds[samp] <- suppressMessages(bootstrap_test_data %>%
                                                       group_by(cell) %>%
                                                       summarize(m = mean(resid_dist[puff == 1])) %>%
                                                       pull(m) %>%
                                                       sd())
        bootstrap_nonpuff_sds[samp] <- suppressMessages(bootstrap_test_data %>%
                                                          group_by(cell) %>%
                                                          summarize(m = mean(resid_dist[puff == 0])) %>%
                                                          pull(m) %>%
                                                          sd())
        bootstrap_puff_sds_est[samp] <- sqrt(summary(bootstrap_me_mod_puff)$varcor[[1]][1])
        bootstrap_nonpuff_sds_est[samp] <- sqrt(summary(bootstrap_me_mod_nonpuff)$varcor[[1]][1])
      }
      #print(samp)
    }
    sd_lm_puff <- lm(bootstrap_puff_sds ~ bootstrap_puff_sds_est)
    sd_lm_nonpuff <- lm(bootstrap_nonpuff_sds ~ bootstrap_nonpuff_sds_est)
    puff_re_sd_new <- coef(sd_lm_puff)[1] + coef(sd_lm_puff)[2]*puff_re_sd
    nonpuff_re_sd_new <- coef(sd_lm_nonpuff)[1] + coef(sd_lm_nonpuff)[2]*nonpuff_re_sd
    ...
    for(samp in 1:num_boot){
      ...
      for(k in 1:num_cells){
        re_puff <- rnorm(1, 0, puff_re_sd_new)
        re_nonpuff <- rnorm(1, 0, nonpuff_re_sd_new)
...
write.table(test_mean_est, file="ls_bootstrap_scenario_3c_adj_estimates.txt")
write.table(bootstrap_mean_est, file="ls_bootstrap_scenario_3c_adj_ci_reps.txt")
