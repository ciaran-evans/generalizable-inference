...
simulate_test_data <- function(n_obs_cell, num_cells = 15){
  ...
  for(k in 1:num_cells){
    ...
    x_k <- z_k + re_k + rnorm(n_obs_cell, 0, 0.2)
    test_data <- test_data %>%
...
write.table(test_mean_est, file="ls_mixture_scenario_1c_fixed_estimates.txt")
write.table(bootstrap_mean_est, file="ls_mixture_scenario_1c_fixed_ci_reps.txt")