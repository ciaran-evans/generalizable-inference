...
full_mix <- stan_model('gauss_mix_hierarchical_fixed.stan')
...
for(ci_rep in 1:num_ci){
  tryCatch({
    ...
    re_sd = best_pars["re_sigma"]
    
    fitted_data <- data.frame(cell = 1:num_cells,
                              fitted_nonpuff_mean = nonpuff_intercept + best_pars[3:(num_cells + 2)],
                              fitted_puff_mean = puff_intercept + best_pars[3:(num_cells + 2)])
...
write.table(test_mean_est, file="ls_mixture_scenario_3c_fixed_estimates.txt")
write.table(bootstrap_mean_est, file="ls_mixture_scenario_3c_fixed_ci_reps.txt")